...
class MultiPdfCropperApp:
    def __init__(self, root, pdf_path):
        self.root = root
        self.root.title("複数枠対応カッター")
        ...
        # --- 3. 状態管理（リスト化） ---
        self.rects = []       # キャンバス上の四角形オブジェクトのリスト
        self.start_x = 0
        self.start_y = 0
    ...
    def on_move(self, event):
        # リストの最後（今描いている枠）を更新
        if self.rects: # エラー回避
            self.canvas.coords(self.rects[-1], self.start_x, self.start_y, event.x, event.y)
        else:
            print("エラー：枠がありません。\n（クリックされてないのに、ドラッグが働いたよ。\nマウスの故障かも？）")
    ...
    def save_all_clips(self):
        if not self.rects:
            messagebox.showwarning("警告", "枠を1つ以上描いてください")
            return

        save_path = filedialog.asksaveasfilename(defaultextension=".pdf", filetypes=[("PDF files", "*.pdf")])
        if not save_path: return # エラー回避。open(save_path, "wb")がエラーになる。
        ...
        for page_in in reader.pages:
            # 各ページに対して、描かれた枠の数だけ新しいページを作る
            for r_id in self.rects:
                # 枠の現在の座標を取得
                coords = self.canvas.coords(r_id) # [x1, y1, x2, y2]
        ...
        messagebox.showinfo("成功", "分割保存が完了しました！")
...
if __name__ == "__main__":
    ...
    if path:
        root.deiconify()
        app = MultiPdfCropperApp(root, path)
        root.mainloop()
    else:
        root.destroy()